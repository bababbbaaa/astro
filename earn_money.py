...
user=user.where(User.ActiveUntil<=date_yesterday)
user=user.where(User.SubscrType_ID==3)
session.query(User).filter(User.SubscrType_ID==3,User.ActiveUntil<=date_yesterday).update({"SubscrType_ID":5,"ActiveUntil":date_yesterday})
session.commit()
session=sessionmaker(engine)()

recurent_subs=session.query(New_Subscription).filter(New_Subscription.End==today).all()
for i in range(len(recurent_subs)):
    ...
    try_rec_in_base(recurent_subs[i].TelegramID)
    # print(pay.text)
    try:
