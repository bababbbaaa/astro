...
sl3_2={30:89,
        180:449,365:745}
sl3_2_text='''Доступно 3 варианта продления подписки: на 30, 180 и 365 дней.

✅ Стоимость подписки на месяц - <strike>99</strike> 89 рублей

✅ Стоимость подписки на пол года - <strike>594</strike> 449 ₽

✅ Стоимость подписки на год: <strike>1188</strike> 745 ₽
...
@dp.callback_query_handler(lambda call: call.data.split(";")[0] == "sl3")
async def sl3_subscribe(call):
    ...
    if sub_type==1 or sub_type==2:
        ...
        but1=types.InlineKeyboardButton(text="Активировать подписку", callback_data="sl3_2opt;"+str(sub_type))
        keyboard.row(but1)
    ...
    if sub_type == 100:
        ...
        but1 = types.InlineKeyboardButton(
            text="Активировать подписку", callback_data="sl3_2opt;"+str(sub_type))
        keyboard.row(but1)
    ...
    if sub_type == 3:
        ...
        but1 = types.InlineKeyboardButton(
            text="Продлить подписку", callback_data="sl3_2opt;"+str(sub_type))
        but2 = types.InlineKeyboardButton(
            text="Отказаться от подписки", callback_data="sl3_end")
...
@dp.callback_query_handler(lambda call: call.data.split(";")[0] == "sl3_sub")
async def sl3_sub_options(call):
    try:
        ...
        if is_new == "n":
            keyboard = types.InlineKeyboardMarkup()
            but1 = types.InlineKeyboardButton(
                text="Да", callback_data="sl3_agr"+";"+str(days))
            but2 = types.InlineKeyboardButton(
                text="Нет", callback_data="sl3_net")
            but = types.InlineKeyboardButton(
                text="Назад", callback_data="sl3_2opt")

            keyboard.row(but1, but2)
            keyboard.add(but)

            await wait_until_send(id, "Стоимость подписки  на "+str(days)+" дней составит"+str(sl3_2[days]) + "рублей.\n\nПеред оплатой подписки, пожалуйста, ознакомьтесь со следующими документами:\n\nСоглашение на обработку персональных данных" +
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call:call.data.find("sl3_offer") != -1)
async def sl3_offert(call):
    try:
        days=call.data.split(";")[1]
        id = call.from_user.id
        keyboard=types.InlineKeyboardMarkup()
        but=InlineKeyboardButton(text="Назад",callback_data="sl3_agr;"+days)
        keyboard.add(but)
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sl3_inf") != -1)
async def sl3_sub_info(call):
    try:
        id = call.from_user.id
        keyboard = types.InlineKeyboardMarkup()
        but = types.InlineKeyboardButton(
...
@dp.callback_query_handler(lambda call: call.data.find("sl3_end") != -1)
async def sl3_end_sub(call):
    try:
        id = call.from_user.id
        keyboard = types.InlineKeyboardMarkup()
        but1 = types.InlineKeyboardButton(
            text="да", callback_data="sl3_fin_sub")
        but2 = types.InlineKeyboardButton(
            text="нет", callback_data="sl3_del")
        keyboard.row(but1, but2)
...
Вы точно хотите отказаться от подписки?''', reply_markup=keyboard)
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sl3_fin_sub")!=-1)
async def sl3_fin_sub(call):
    try:
        id = call.from_user.id
        delete_sub(id=id)
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sl3_del") != -1)
async def sl3_delete(call):
    try:
        id = call.from_user.id
...
async def sl3_fin_end(call):
    try:
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sl3_SUBSCR_ACT") != -1)#sl3_SUBSCR_ACT
async def sl3_active_sub(call):
    id=call.from_user.id
...
@dp.callback_query_handler(lambda call: call.data.find("sl3_ret") != -1)
async def sl3_ret(call):
    try:
        id = call.from_user.id
        keyboard = types.InlineKeyboardMarkup()
        but1 = types.InlineKeyboardButton(
            text="30 дней", callback_data="sl3_sub;n;30")
        but2 = types.InlineKeyboardButton(
            text="180 дней", callback_data="sl3_sub;n;180")
        but3 = types.InlineKeyboardButton(
            text="365 дней", callback_data="sl3_sub;n;365")
        but4 = types.InlineKeyboardButton(
            text="Что входит в подписку?", callback_data="sl3_inf")
        but5 = types.InlineKeyboardButton(
            text="Назад", callback_data="sl3_full_back")
...
@dp.callback_query_handler(lambda call: call.data.find("sl3_full_back") != -1)
async def sl3_full_back(call):
    id = call.from_user.id
    await wait_until_send(
...
@dp.callback_query_handler(lambda call: call.data.find("sl3_2opt") != -1)
async def sl3_opt2(call):
    try:
        id=call.from_user.id
        sub_type=int(functions.GetUsers(id)[0]["SubscrType_ID"])
        ...
        keyboard = types.InlineKeyboardMarkup()
        but1 = types.InlineKeyboardButton(
            text="30 дней", callback_data="sl3_agr;30")
        but2 = types.InlineKeyboardButton(
            text="180 дней", callback_data="sl3_agr;180")
        but3 = types.InlineKeyboardButton(
            text="365 дней", callback_data="sl3_agr;365")
        but4 = types.InlineKeyboardButton(
            text="Что входит в подписку?", callback_data="sl3_inf")
        but5 = types.InlineKeyboardButton(
            text="Назад", callback_data="sl3_full_back")
        ...
        keyboard.add(but5)
        if sub_type==1:
            await wait_until_send(id,'Спасибо, что пользуетесь ботом "Твой гороскоп"\n\nВ данный момент у вас действует пробный период, подписки.'+ sl3_2_text,reply_markup=keyboard,parse_mode="html")     
        elif sub_type==2 or sub_type==4 or sub_type==5 or sub_type==3:
            await wait_until_send(id,sl3_2_text,reply_markup=keyboard,parse_mode="html")
        # elif sub_type==2:
...
@dp.callback_query_handler(lambda call: call.data.find("sl3_agr") != -1)
async def sl3_agreement(call):
    try:
        ...
        url=for_payments.make_recurse_pay(id=id,days=days,amount=sl3_2[days],test=0)
        keyboard=types.InlineKeyboardMarkup()
        
        but1=types.InlineKeyboardButton(text="Назад",callback_data="sl3_2opt;"+str(days))
        but=types.InlineKeyboardButton(text="Оплата",url=url)
        but2=types.InlineKeyboardButton(text="Офферта",callback_data="sl3_offer;"+str(days))
        keyboard.row(but,but2)
        keyboard.add(but1)
        mess=await wait_until_send(id, "Стоимость подписки  на "+str(days)+" дней составит "+str(sl3_2[days]) + " рублей.", reply_markup=keyboard)
        if id in delete_cache:
            for i in delete_cache[id]:
                bot.delete_message(id, i)
        delete_cache[id] = []
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sl3_net") != -1)
async def sl3_no_agr(call):
    try:
        id = call.from_user.id
