...
async def registartion(id:int,text:str):
    ...
    if user["Name"] == "":
        if functions.validation_everything(type="Name",text=text)[0]==True:
            horoscopeusr.ChUserInfo(inpTelegramID=id,inpFieldName="Name",inpValue=text)
        ...
        if functions.validation_everything(type="Birthday",text=text)[0]==True:
            horoscopeusr.ChUserInfo(inpTelegramID=id,inpFieldName="Birthday",inpValue=text)
        ...
        if functions.validation_everything(type="birth_place",text=text)[0]==True:
            horoscopeusr.ChUserInfo(inpTelegramID=id,inpFieldName="Birthplace",inpValue=text)
        ...
        if functions.validation_everything(type="BirthTime",text=text)[0]==True:
            ...
            horoscopeusr.ChUserInfo(inpTelegramID=id,inpFieldName="BirthTime",inpValue=text)
            keyboard1 = types.InlineKeyboardMarkup()
            keyboard1.row(types.InlineKeyboardButton(text="Утро",callback_data="tim;mor"),types.InlineKeyboardButton(text="Вечер",callback_data="tim;evn"))
...
async def time_select(call):
    try:
        ...
        bot.delete_message(chat_id=id,message_id=call.message.message_id)#Удаляем сообщение, чтобы юзер не игрался с выбором времени
