...
sal_1={30:79,
        180:399,365:662}
sal_1_text='''Доступно 3 варианта продления подписки: на 30, 180 и 365 дней.

✅ Стоимость подписки на месяц -<strike>99</strike> 79 рублей

✅ Стоимость подписки на пол года - <strike>594</strike> 399 ₽

✅ Стоимость подписки на год: <strike>1188</strike> 662 ₽

При продлении сроки подписок
суммируются.

Выбери подходящий тариф!'''
@dp.callback_query_handler(lambda call: call.data.split(";")[0] == "sal")
async def sal_subscribe(call):
    ...
    if sub_type == 3:
        ...
        but1 = types.InlineKeyboardButton(
            text="Продлить подписку", callback_data="sal_2opt;"+str(sub_type))
        but2 = types.InlineKeyboardButton(
            text="Отказаться от подписки", callback_data="sal_end")
...
@dp.callback_query_handler(lambda call: call.data.split(";")[0] == "sal_sub")
async def sal_sub_options(call):
    try:
        ...
        if is_new == "n":
            keyboard = types.InlineKeyboardMarkup()
            but1 = types.InlineKeyboardButton(
                text="Да", callback_data="sal_agr"+";"+str(days))
            but2 = types.InlineKeyboardButton(
                text="Нет", callback_data="sal_net")
            but = types.InlineKeyboardButton(
                text="Назад", callback_data="sal_2opt")

            keyboard.row(but1, but2)
            keyboard.add(but)

            await wait_until_send(id, "Стоимость подписки  на "+str(days)+" дней составит"+str(sal_1[days]) + "рублей.\n\nПеред оплатой подписки, пожалуйста, ознакомьтесь со следующими документами:\n\nСоглашение на обработку персональных данных" +
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call:call.data.find("sal_offer") != -1)
async def sal_offert(call):
    try:
        days=call.data.split(";")[1]
        id = call.from_user.id
        keyboard=types.InlineKeyboardMarkup()
        but=InlineKeyboardButton(text="Назад",callback_data="sal_agr;"+days)
        keyboard.add(but)
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sal_inf") != -1)
async def sal_sub_info(call):
    try:
        id = call.from_user.id
        keyboard = types.InlineKeyboardMarkup()
        but = types.InlineKeyboardButton(
...
@dp.callback_query_handler(lambda call: call.data.find("sal_end") != -1)
async def sal_end_sub(call):
    try:
        id = call.from_user.id
        keyboard = types.InlineKeyboardMarkup()
        but1 = types.InlineKeyboardButton(
            text="да", callback_data="sal_fin_sub")
        but2 = types.InlineKeyboardButton(
            text="нет", callback_data="sal_del")
        keyboard.row(but1, but2)
...
Вы точно хотите отказаться от подписки?''', reply_markup=keyboard)
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sal_fin_sub")!=-1)
async def sal_fin_sub(call):
    try:
        id = call.from_user.id
        delete_sub(id=id)
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sal_del") != -1)
async def sal_delete(call):
    try:
        id = call.from_user.id
...
async def sal_fin_end(call):
    try:
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sal_SUBSCR_ACT") != -1)#sal_SUBSCR_ACT
async def sal_active_sub(call):
    id=call.from_user.id
...
@dp.callback_query_handler(lambda call: call.data.find("sal_ret") != -1)
async def sal_ret(call):
    try:
        id = call.from_user.id
        keyboard = types.InlineKeyboardMarkup()
        but1 = types.InlineKeyboardButton(
            text="30 дней", callback_data="sal_sub;n;30")
        but2 = types.InlineKeyboardButton(
            text="180 дней", callback_data="sal_sub;n;180")
        but3 = types.InlineKeyboardButton(
            text="365 дней", callback_data="sal_sub;n;365")
        but4 = types.InlineKeyboardButton(
            text="Что входит в подписку?", callback_data="sal_inf")
        but5 = types.InlineKeyboardButton(
            text="Назад", callback_data="sal_full_back")
...
@dp.callback_query_handler(lambda call: call.data.find("sal_full_back") != -1)
async def sal_full_back(call):
    id = call.from_user.id
    await wait_until_send(
...
@dp.callback_query_handler(lambda call: call.data.find("sal_2opt") != -1)
async def sal_opt2(call):
    try:
        id=call.from_user.id
        sub_type=int(functions.GetUsers(id)[0]["SubscrType_ID"])
        ...
        keyboard = types.InlineKeyboardMarkup()
        but1 = types.InlineKeyboardButton(
            text="30 дней", callback_data="sal_agr;30")
        but2 = types.InlineKeyboardButton(
            text="180 дней", callback_data="sal_agr;180")
        but3 = types.InlineKeyboardButton(
            text="365 дней", callback_data="sal_agr;365")
        but4 = types.InlineKeyboardButton(
            text="Что входит в подписку?", callback_data="sal_inf")
        but5 = types.InlineKeyboardButton(
            text="Назад", callback_data="sal_full_back")
        ...
        keyboard.add(but5)
        if sub_type==1:
            await wait_until_send(id,'Спасибо, что пользуетесь ботом "Твой гороскоп"\n\nВ данный момент у вас действует пробный период, подписки.'+ sal_1_text,reply_markup=keyboard,parse_mode="html")     
        elif sub_type==2 or sub_type==4 or sub_type==5 or sub_type==3:
            await wait_until_send(id,sal_1_text,reply_markup=keyboard,parse_mode="html")
        # elif sub_type==2:
...
@dp.callback_query_handler(lambda call: call.data.find("sal_agr") != -1)
async def sal_agreement(call):
    try:
        ...
        url=for_payments.make_recurse_pay(id=id,days=days,amount=sal_1[days],test=0)
        keyboard=types.InlineKeyboardMarkup()
        
        but1=types.InlineKeyboardButton(text="Назад",callback_data="sal_2opt;"+str(days))
        but=types.InlineKeyboardButton(text="Оплата",url=url)
        but2=types.InlineKeyboardButton(text="Офферта",callback_data="sal_offer;"+str(days))
        keyboard.row(but,but2)
        keyboard.add(but1)
        mess=await wait_until_send(id, "Стоимость подписки  на "+str(days)+" дней составит "+str(sal_1[days]) + " рублей.", reply_markup=keyboard)
        if id in delete_cache:
            for i in delete_cache[id]:
                bot.delete_message(id, i)
        delete_cache[id] = []
        ...
        try:
            ...
            pass
@dp.callback_query_handler(lambda call: call.data.find("sal_net") != -1)
async def sal_no_agr(call):
    try:
        id = call.from_user.id
